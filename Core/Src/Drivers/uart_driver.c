...
void UART2_Init(uint32_t baudrate, uint32_t word_length, uint32_t parity, uint32_t stop_bits) {
    ...
    // 6. Cấu hình Baud Rate (Sử dụng PCLK1_FREQUENCY_HZ từ rcc_config.h)
    uint32_t pclk1_freq = PCLK1_FREQUENCY_HZ; // Lấy từ rcc_config.h
    uint32_t usartdiv_100 = (pclk1_freq * 100U) / (baudrate * 16U);
    uint32_t mantissa = usartdiv_100 / 100U;
    uint32_t fraction_x100 = usartdiv_100 % 100U;
    uint32_t fraction_reg = (((fraction_x100 * 16U) + 50U) / 100U) & 0x0FU;
    USART2->BRR = (mantissa << 4) | fraction_reg;
...
bool UART2_ReadByte_FromBuffer(uint8_t* data) {
    ...
    __disable_irq(); // Critical section để đọc buffer
...
void USART2_IRQHandler(void) {
    ...
    if ((sr_reg & USART_SR_RXNE) && (USART2->CR1 & USART_CR1_RXNEIE)) {
        ...
        if (g_uart2_rx_buffer_count < UART2_RX_BUFFER_SIZE) {
            ...
            // Lỗi: RX Buffer bị đầy!
            // Có thể set một cờ lỗi hoặc xử lý khác.
        }
    ...
    if (sr_reg & (USART_SR_ORE | USART_SR_NE | USART_SR_FE | USART_SR_PE)) {
        // Để xóa các cờ lỗi này, thường cần đọc SR rồi đọc DR.
        // Việc đọc DR ở ngắt RXNE đã thực hiện điều này cho ORE.
        // Nếu RXNE không xảy ra cùng lúc, cần đọc DR ở đây.
        volatile uint32_t temp_dr = USART2->DR; // Đọc DR để xóa cờ ORE, NE, FE
